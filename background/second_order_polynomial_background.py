...
def signal_model(t, c1, c2, scale_factor, s_intra):
    return background_model(t, c1, c2) * (1 + scale_factor * (s_intra - 1))
...
class SecondOrderPolynomialBackground(Background):
    """Background model: second-order polynom."""
    ...
    def get_fit(self, t, background_parameters, s_intra):
        return signal_model(
            t, background_parameters["c1"], background_parameters["c2"], background_parameters["scale_factor"], s_intra
            )
    
    
    def get_background(self, t, background_parameters, modulation_depth = 0):
        return background_model(t, background_parameters["c1"], background_parameters["c2"]) * \
