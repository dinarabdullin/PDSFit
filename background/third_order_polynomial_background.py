...
def signal_model(t, c1, c2, c3, scale_factor, s_intra):
    return background_model(t, c1, c2, c3) * (1 + scale_factor * (s_intra - 1))
...
class ThirdOrderPolynomialBackground(Background):
    """Background model: third-order polynom."""
    ...
    def get_fit(self, t, background_parameters, s_intra):
        return signal_model(
            t, background_parameters["c1"], background_parameters["c2"], 
            background_parameters["c3"], background_parameters["scale_factor"], s_intra
            )
