...
class Ridme_5p_rect(Experiment):
    """Five-pulse RIDME experiment with rectangular pulses."""
    ...
    def detection_probability(self, resonance_frequencies):
        ...
        # This equation needs to be validated theoretically!
        detection_probabilities = (self.bandwidth_detection_pi_half_pulse / rabi_frequencies_pi_half_pulse)**3 * \
            np.sin(2 * np.pi * rabi_frequencies_pi_half_pulse * self.detection_pi_half_pulse_length)**3 * \
    ...
    def pump_probability(self, T1, g_anisotropy, g_eff):
        """Compute pump probabilities for given g-values."""
        if g_anisotropy:
    ...
    def get_detection_bandwidth(self, ranges=()):
        ...
        detection_bandwidth = {}
        detection_bandwidth["freq"] = frequencies
        detection_bandwidth["prob"] = probabilities
